...
from beginner_tutorials.msg import Action
...
def main_page():
  ...
  return render_template('/main.html', message=page.actions[page.current_action],
    display_type=page.test_mode, people=page.people, transcript=page.transcript, speech=speech, face=page.current_face)
...
def action_callback(data):
  page.current_action=data.Action
  page.current_face=data.Mood
  person_involved=data.Person
  speech = [ (x, round(float(y)/page.total_speech*100)) for x,y in page.speech]
  return render_template('/main.html', message=page.actions[page.current_action], 
    display_type=page.test_mode, people=page.people, transcript=page.transcript, speech=speech, face=page.current_face)
...
def listener():
  ...
  rospy.Subscriber("Action", Action, action_callback)
  rospy.spin()
